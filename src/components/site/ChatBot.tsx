    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content: res.reply,
          recommendations: res.recommendations as SchemeRec[] | undefined,
        },
      ]);
    } catch (e) {
                ...
                <div
                  className={
                    m.role === "user"
                      ...
                      : "max-w-[95%] space-y-2.5 rounded-2xl rounded-bl-sm bg-card border border-border/80 px-3.5 py-2.5 text-sm text-card-foreground shadow-xs"
                  }
                >
                  ...
                  {m.recommendations && m.recommendations.length > 0 && (
                    <div className="mt-3 space-y-2 border-t border-border/60 pt-2.5">
                      <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                        ...
                        Verified Government Recommendations ({m.recommendations.length})
                      </p>
                      <div className="grid gap-2">
                        {m.recommendations.map((rec) => {
                          const targetUrl = rec.officialUrl || rec.fallbackUrl;
                          const isOfficial =
                            !rec.officialUrl?.includes("myscheme.gov.in") && !!rec.officialUrl;
                          return (
                            <div
                              key={rec.id}
                              className="rounded-xl border border-border/70 bg-background/80 p-2.5 transition hover:border-primary/50 hover:bg-background"
                            >
                              <div className="flex items-center justify-between gap-1">
                                ...
                                <span className="text-[10px] font-medium text-muted-foreground">
                                  {rec.state ? rec.state : "Central"}
                                </span>
                              </div>
                              <h4 className="mt-1 font-semibold text-xs text-foreground line-clamp-1">
                                {rec.name}
                              </h4>
                              ...
                              {targetUrl && (
                                <a
                                  ...
                                  className="mt-2 inline-flex items-center gap-1 rounded-lg bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary hover:bg-primary hover:text-primary-foreground transition"
                                >
                                  {isOfficial ? "Apply on Official Portal" : "View on myScheme"}
