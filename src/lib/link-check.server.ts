...
export async function checkOfficialLink(
  ...
  if (!res) {
    ...
    if (/ENOTFOUND|getaddrinfo|dns|name not resolved/i.test(message)) {
      return {
        status: "invalid",
        httpStatus: null,
        note: "Website no longer exists (host not found)",
      };
    }
  ...
  if (res.status === 404 || res.status === 410) {
    return {
      ...
      note: `Page no longer exists (${res.status})`,
    };
  }
  ...
  return {
    status: "unreachable",
    httpStatus: res.status,
    note: `Could not reach (HTTP ${res.status})`,
  };
}
