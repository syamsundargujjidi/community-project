...
export type MatchStatus = "eligible" | "verify" | "ineligible" | "out_of_scope" | "inactive";
  ...
  switch (scope) {
    case "CENTRAL_NATIONWIDE":
      return {
        ...
        detail: "Central scheme — available across India",
      };
    case "CENTRAL_STATE_IMPLEMENTED":
      if (states.length === 0 || states.includes(user))
        return {
          ok: true,
          label: "Government level",
          detail: "Central scheme implemented through your State/UT",
        };
      return {
        ok: false,
        label: "State",
        detail: `Implemented only in ${(scheme as ScopedScheme).available_states?.join(", ")}`,
      };
    case "MULTI_STATE":
      ...
      return {
        ok: false,
        label: "State",
        detail: `Available only in ${(scheme as ScopedScheme).available_states?.join(", ")}`,
      };
      ...
      if (districts.length === 0 || !userDistrict || districts.includes(userDistrict))
        return {
          ...
          detail: districts.length
            ? `Districts: ${(scheme as ScopedScheme).districts?.join(", ")}`
            : `${p.state} — matched`,
        };
      return {
        ok: false,
        label: "District",
        detail: `Only for ${(scheme as ScopedScheme).districts?.join(", ")}`,
      };
    }
    case "UT_ONLY":
    case "STATE_ONLY":
...
function compare(op: string, actual: unknown, expected: unknown): RuleOutcome {
  ...
  switch (op) {
    ...
    case "boolean":
    case "is_true":
      return Boolean(actual) === (expected === undefined ? true : Boolean(expected))
        ? "pass"
        : "fail";
    case "exists":
      return "pass";
...
function evalRule(rule: Rule, p: UserProfile): { outcome: RuleOutcome; detail: string } {
  ...
  const detail = outcome === "unknown" ? `${label}` : `${label} (yours: ${String(actual)})`;
...
export function evaluateScheme(scheme: Scheme, p: UserProfile): SchemeMatch {
  ...
  if (scheme.min_age != null || scheme.max_age != null) {
    ...
    if (!p.age || Number.isNaN(p.age)) {
      add("age", "Age", "unknown", `Required age ${range}`);
    } else if (
  ...
  if (scheme.max_annual_income != null) {
    if (p.annualIncome == null || Number.isNaN(p.annualIncome)) {
      add(
        ...
        "Annual income",
        "unknown",
        `Limit ${inr(scheme.max_annual_income)} — income not provided`,
      );
      ...
      add(
        ...
        "Annual income",
        "pass",
        `${inr(p.annualIncome)} — limit ${inr(scheme.max_annual_income)}`,
      );
      ...
      add(
        ...
        "Annual income",
        "fail",
        `${inr(p.annualIncome)} exceeds limit ${inr(scheme.max_annual_income)}`,
      );
    }
  }
  if (s.min_annual_income != null) {
    if (p.annualIncome == null || Number.isNaN(p.annualIncome)) {
      ...
      add(
        ...
        "Minimum income",
        "pass",
        `${inr(p.annualIncome)} — minimum ${inr(s.min_annual_income)}`,
      );
    } else {
      add(
        "income_min",
        "Minimum income",
        "fail",
        `${inr(p.annualIncome)} below minimum ${inr(s.min_annual_income)}`,
      );
    }
  }
    ...
    if (r === "yes" || r === "any" || r === "empty") {
      ...
      if (viaParent === "yes" || viaParent === "any" || viaParent === "empty") {
        add(
          ...
          "Occupation",
          "pass",
          `Parent occupation: ${p.parentOccupation} — matched`,
        );
        reasons.push("occupationMatches");
      } else {
  ...
  // ---- Caste category (OR list) --------------------------------------
  {
    ...
    if (r === "yes") {
      ...
      add(
        ...
        "Category",
        "fail",
        `For ${(s.caste_categories ?? []).join(", ").toUpperCase()}`,
      );
    }
  }
  ...
  boolCheck(
    ...
    farmerValue,
    "Farmer — matched",
    "Farmer status not provided",
  );
  boolCheck(
    "land",
    "Land ownership",
    s.land_ownership_required,
    anyP["ownsLand"],
    "Owns cultivable land",
    "Land ownership not provided",
  );
  boolCheck(
    "bpl",
    "BPL status",
    s.bpl_only,
    anyP["isBpl"],
    "BPL household — matched",
    "BPL / ration card status not provided",
  );
  boolCheck(
    "widow",
    "Widow",
  ...
  boolCheck(
    ...
    "Minority",
    s.minority_required,
    p.caste ? norm(p.caste) === "minority" : undefined,
    "Minority — matched",
    "Social category not provided",
  );
  ...
  if (s.employment_status && norm(s.employment_status) !== "any") {
    ...
    if (!val)
      add(
        ...
        "Employment status",
        "unknown",
        `Required ${s.employment_status} — not provided`,
      );
    else if (norm(val) === norm(s.employment_status))
      add("employment", "Employment status", "pass", `${val} — matched`);
  ...
  if (s.marital_status && norm(s.marital_status) !== "any") {
    ...
    if (!val)
      add("marital", "Marital status", "unknown", `Required ${s.marital_status} — not provided`);
    else if (norm(val) === norm(s.marital_status))
      add("marital", "Marital status", "pass", `${val} — matched`);
  ...
  if (s.eligibility_rules && typeof s.eligibility_rules === "object") {
    ...
    add(
      "rules",
      "Scheme conditions",
      r.outcome === "pass" ? "pass" : r.outcome === "fail" ? "fail" : "unknown",
      r.detail,
    );
  }
  ...
  if (status === "eligible") {
    const base = 65;
    const bonus = total === 0 ? 20 : Math.round((passed / total) * 30);
    confidence = Math.min(
      99,
      base + bonus + (scheme.is_popular ? 5 : 0) + (isCentral(scheme) ? 0 : 4),
    );
  } else {
