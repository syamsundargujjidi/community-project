...
export const Route = createFileRoute("/_authenticated/dashboard")({
  ssr: false,
  head: () => ({
    meta: [{ title: "Dashboard — Scheme Sathi AI" }, { name: "robots", content: "noindex" }],
  }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      onSnapshot(
        query(collection(db, "users", uid, "savedSchemes"), orderBy("savedAt", "desc"), limit(6)),
        (s) => setSaved(s.docs.map((d) => ({ id: d.id, ...d.data() }))),
      ),
      onSnapshot(
        query(
          collection(db, "users", uid, "eligibilityHistory"),
      ...
      onSnapshot(
        query(collection(db, "users", uid, "recentSchemes"), orderBy("viewedAt", "desc"), limit(6)),
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-12">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Your schemes, history and recommendations in one place.
          </p>
        </div>
      ...
      <div className="card-elevated mt-8 flex flex-wrap items-center gap-4 p-5">
        <div className="flex-1 min-w-[240px]">
          <div className="flex items-center justify-between text-sm font-semibold">
            ...
            <span className="text-primary font-bold">
              {pct > 0 ? `${pct}% provided` : "Get started"}
            </span>
          </div>
        ...
        <Link
          to="/profile"
          className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:brightness-110 transition"
        >
          {pct > 0 ? "Manage profile" : "Provide details"} <ArrowRight className="h-4 w-4" />
        </Link>
      </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Stat
          icon={Bookmark}
          label="Saved schemes"
          value={profile?.savedSchemesCount ?? saved.length}
        />
        <Stat
          icon={ClipboardCheck}
          label="Eligibility checks"
          value={profile?.eligibilityCount ?? history.length}
        />
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-2">
        ...
        >
          {history.length === 0 ? (
            <Empty
              text="Run your first eligibility check."
              linkText="Check now →"
              to="/questionnaire"
            />
          ) : (
                ...
                <li key={h.id} className="rounded-lg border border-border/60 p-3 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="font-medium">
                      {h.eligibleSchemes?.length ?? 0} eligible schemes
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {h.timestamp ? new Date(tsToMs(h.timestamp)).toLocaleDateString() : ""}
                    </span>
                  </div>
            ...
            <ul className="space-y-2">
              {recents.map((r) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">
                    {r.viewedAt ? new Date(tsToMs(r.viewedAt)).toLocaleDateString() : ""}
                  </span>
                </li>
              ))}
            </ul>
              ...
              {searches.map((s) => (
                <li key={s.id} className="rounded-lg border border-border/60 p-3 text-sm">
                  ...
                  <span className="ml-2 text-xs text-muted-foreground">
                    {s.timestamp ? new Date(tsToMs(s.timestamp)).toLocaleString() : ""}
                  </span>
                </li>
              ))}
            </ul>
        ...
        <Panel title="Notifications" icon={Bell}>
          <p className="text-sm text-muted-foreground">
            You're all caught up 🎉 We'll notify you when new schemes matching your profile are
            added.
          </p>
        </Panel>
      </div>
