...
function buildReportCsv(schemes: Scheme[]): string {
  ...
  const rows = schemes.map((s) => {
    ...
    const missingRules =
      ...
      !anyS["eligibility_rules"];
    return [
      s.slug,
      s.name,
      anyS["government_level"],
      anyS["scheme_scope"],
      s.state,
      s.category,
      anyS["scheme_status"] ?? "Active",
      anyS["verification_status"],
      anyS["official_source_url"] || s.official_website || s.apply_url || "",
      anyS["link_status"] ?? "unchecked",
      anyS["link_http_status"] ?? "",
...
function DatabaseDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const quality = {
      verified: schemes.filter(
        (s) => (s as { verification_status?: string }).verification_status === "Verified",
      ).length,
      needsVerification: schemes.filter(
        (s) => (s as { verification_status?: string }).verification_status !== "Verified",
      ).length,
      inactive: schemes.filter(
        (s) => ((s as { scheme_status?: string }).scheme_status ?? "Active") !== "Active",
      ).length,
  ...
  const { data: job, refetch: refetchJob } = useQuery({
    ...
    queryFn: async () => {
      try {
        const snap = await getDoc(doc(getDb(), "jobs", "link_validation"));
